...
public:
	...
	// ゲージ等の水平フィル切り抜き比率をセット
	// ・ratio			… 0.0～1.0。UV.xがこの値を超えるピクセルはピクセルシェーダー側でdiscardされる
	//                    (1.0を指定すると実質的に切り抜きなし)
...
private:
	...
	struct cbSprite {
		...
		float				FillRatio = 1.0f;	// ゲージ用切り抜き比率(0.0～1.0 / 1.0で切り抜きなし)
		float				_pad[3] = {};		// 16バイト境界合わせ用パディング
	};
