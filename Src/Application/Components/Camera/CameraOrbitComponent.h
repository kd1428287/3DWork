...
// SetLookDelta()経由で行う。
//
// ロックオン中の「対象を向く」処理はここでは持たない。以前はここで
// yaw/pitchを対象方向へ直接書き換えていたが、その値はカメラの"位置"
// (CameraFollowComponent::PostUpdate()のオフセット基準)にも使われて
// いるため、位置までロック対象方向へ引っ張られてしまい、カメラ・
// プレイヤー・ロック対象が一直線に並んでプレイヤーが対象を隠す不具合の
// 原因になっていた。ロックオン中の「向き」はCameraFollowComponent側で
// 位置とは独立に計算する(そちらのコメント参照)。
// ============================================================
