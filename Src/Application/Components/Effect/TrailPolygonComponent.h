...
public:
	...
	// 記録中のみ、毎フレーム現在の中心座標・幅ベクトルをトレイルへ追加する。
	void Update(float /*deltaTime*/) override
	{
		if (!emitting_ || !transform_) { return; }

		const Math::Matrix& mWorld = transform_->GetWorldMatrix();
		...
		Math::Vector3 widthVector = worldTip - worldRoot; // 帯の全幅を表すベクトル(長さ=root~tip間の距離)

		m_trailPolygon->AddPoint(center, widthVector);
	}
	...
	{
		// 頂点が2未満(=ポイントが2つ未満)の場合はRebuildVertexArray側で
